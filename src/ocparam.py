...
#            self.ocx_coefs=np.array([0.3255,-2.7677,2.4409,-1.1288,-0.4990])
            ...
            self.ocx_bid=(24,44,52,70)
            self.ocx_coefs=np.array([0.32814,-3.20725,3.22969,-1.36769,-0.81739])
        elif self.sensor == 'HYPSO_HSI':
            self.ocx_bid=(5, 20, 25, 38) # Indices corresponding to Channel IDs for 440, 490, 510 and 555 nm
            self.ocx_coefs=np.array([0.32814,-3.20725,3.22969,-1.36769,-0.81739]) # TODO: These coeffs are just copied from OCI, need testing to see if HYPSO-specific coeffs are needed
