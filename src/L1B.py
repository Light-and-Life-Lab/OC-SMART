...
class L1B(object):
    ...
    def __init__(self, sensorinfo, L1Bname, GEOpath=None):
        ...
        if self.sensor in ['OLI','OLI2']:
            #find MTL file
            l8mtl_path=glob(self.l1bpath + '/*MTL.txt')[0]
            self.l8metadata=load_mtl(l8mtl_path)
            metadataroot=list(self.l8metadata.keys())

            if metadataroot[0]=='L1_METADATA_FILE':
                self.l8collection=1
            elif metadataroot[0]=='LANDSAT_METADATA_FILE':
                self.l8collection=2                 
        if self.sensor in ['S2A', 'S2B']:
            #read the xml file
            #self.s2_path = self.l1bname+'/GRANULE/'+listdir(self.l1bname+'/GRANULE/')[0]
            self.s2_path = glob(self.l1bname+'/GRANULE/*')[0]
            ...
            self.s2_l1cpath = self.l1bname+'/MTD*.xml'
            l1xml_path = glob(self.s2_l1cpath)[0]
            self.l1xmlroot = objectify.parse(l1xml_path).getroot()

    def parse_datetime(self):
        # Find time info from attribute of NetCDF file if available, or otherwise by parsing L1B filename
        
        l1b_basename = basename(self.l1bname)
        if self.sensor == 'EPIC':
            self.datestr = l1b_basename[8:22]
            self.year = l1b_basename[8:12]
            self.month = l1b_basename[12:14]
            self.day = l1b_basename[14:16]
            #self.l1btime=datetime.strptime(self.datestr,'%Y%m%d%H%M%S').timestamp()
        elif self.sensor == 'SGLI':
            self.year = l1b_basename[7:11]
            self.month = l1b_basename[11:13]
            self.day = l1b_basename[13:15]

            f = h5py.File(self.l1bpath + '/'+l1b_basename, 'r')
            ...
            self.datestr = l1b_basename[7:15] + hour + minute + second
            f.close()
            #self.l1btime=datetime.strptime(self.datestr,'%Y%m%d%H%M%S').timestamp() 
        elif self.sensor == 'VIIRS':
            if self.datasource == 'LAADS DAAC':
                self.year = l1b_basename[10:14]
                doy = int(l1b_basename[14:17])
                dt = datetime(int(self.year), 1, 1) + timedelta(doy - 1)
                self.month = '{:02d}'.format(dt.month)
                self.day = '{:02d}'.format(dt.day)                
                self.datestr = self.year + self.month + self.day + l1b_basename[18:22] + '00'
            ...
            posn = l1b_basename.find('____')+4
            self.year = l1b_basename[posn:posn+4]
            self.month = l1b_basename[posn+4:posn+6]
            self.day = l1b_basename[posn+6:posn+8]

            f = h5py.File(self.l1bpath+'/'+l1b_basename+'/time_coordinates.nc', 'r')
            ...
            if self.datasource == 'LAADS DAAC':
                ...
                if self.sensor == 'MODIS-Aqua':
                    self.year = l1b_basename[11:15]
                    self.month = l1b_basename[15:17]
                    self.day = l1b_basename[17:19]
                    dt = datetime(int(self.year), int(self.month), int(self.day))
                    self.datestr = self.year + self.month + self.day + l1b_basename[20:26]
                else:
                    self.year = l1b_basename[12:16]
                    self.month = l1b_basename[16:18]
                    self.day = l1b_basename[18:20]
                    dt = datetime(int(self.year), int(self.month), int(self.day))
                    self.datestr = self.year + self.month + self.day + l1b_basename[21:27]
        elif self.sensor in ['OLI','OLI2']:
             #find MTL file
            l8mtl_path = glob(self.l1bpath + '/*MTL.txt')[0]
            self.l8metadata = load_mtl(l8mtl_path)
            l8metadataroot = list(self.l8metadata.keys())

            if l8metadataroot[0] == 'L1_METADATA_FILE':
            ...
            elif l8metadataroot[0] == 'LANDSAT_METADATA_FILE':
                idx = self.l8metadata['LANDSAT_METADATA_FILE']['IMAGE_ATTRIBUTES']['SCENE_CENTER_TIME'].find('.')
                dt = time.strptime(self.l8metadata['LANDSAT_METADATA_FILE']['IMAGE_ATTRIBUTES']['DATE_ACQUIRED'] + self.l8metadata['LANDSAT_METADATA_FILE']['IMAGE_ATTRIBUTES']['SCENE_CENTER_TIME'][0:idx],'%Y-%m-%d%H:%M:%S')
            ...
            #read the xml file
            #self.s2_path = self.l1bpath+'/'+l1b_basename+'/GRANULE/'+listdir(self.l1bpath+'/'+l1b_basename+'/GRANULE/')[0]
            self.s2_path = glob(self.l1bpath + '/' + l1b_basename + '/GRANULE/*')[0]
            xml_path = glob(self.s2_path + '/*.xml')[0]
            self.s2xmlroot = objectify.parse(xml_path).getroot()
            ...
            if self.datasource == 'OBPG':                
                self.year = l1b_basename[1:5]
                doy = int(l1b_basename[5:8])
                dt = datetime(int(self.year), 1, 1) + timedelta(doy - 1)
                self.month = '{:02d}'.format(dt.month)
                self.day = '{:02d}'.format(dt.day)
                self.datestr = self.year + self.month + self.day + l1b_basename[8:14]  
        elif self.sensor == 'SeaWiFS':
            dt = time.strptime(l1b_basename[1:14], '%Y%j%H%M%S')
            self.year = '{:04d}'.format(dt.tm_year)
            self.month = '{:02d}'.format(dt.tm_mon)
            self.day = '{:02d}'.format(dt.tm_mday)
            hour = '{:02d}'.format(dt.tm_hour)
            ...
            with h5py.File(self.l1bpath+'/' + l1b_basename, 'r') as f:
                start_time_unix = f['/metadata/adcs/timestamps'][0]
                end_time_unix = f['/metadata/adcs/timestamps'][-1]

                midpoint_time_unix = (start_time_unix + end_time_unix)/2
                datetime_struct = time.gmtime(midpoint_time_unix)

                self.year = '{:04d}'.format(datetime_struct.tm_year)
                self.month = '{:02d}'.format(datetime_struct.tm_mon)
                self.day = '{:02d}'.format(datetime_struct.tm_mday)
                hour = '{:02d}'.format(datetime_struct.tm_hour)
                minute ='{:02d}'.format(datetime_struct.tm_min)
                second = '{:02d}'.format(datetime_struct.tm_sec) 
                self.datestr = self.year + self.month + self.day + hour + minute + second
    ...
    def readgeo(self):               
        if self.sensor=="EPIC": 
            ...
            lat_grid_ymesh,lat_grid_xmesh=np.meshgrid(lat_grid_yt,lat_grid_xt)
            func=interpolate.RegularGridInterpolator((lat_grid_y,lat_grid_x),lat.T,method='linear')
            # self.latitude=func((lat_grid_yt,lat_grid_xt))
            self.latitude=func((np.reshape(lat_grid_ymesh,(self.dim[0]*self.dim[1])), np.reshape(lat_grid_xmesh,(self.dim[0]*self.dim[1])))).reshape(self.dim[0],self.dim[1])
            ...
            lon_grid_ymesh,lon_grid_xmesh=np.meshgrid(lon_grid_yt,lon_grid_xt)
            func=interpolate.RegularGridInterpolator((lon_grid_y,lon_grid_x),lon.T,method='linear')
            # self.longitude=func(lon_grid_yt,lon_grid_xt)
            self.longitude=func((np.reshape(lon_grid_ymesh,(self.dim[0]*self.dim[1])), np.reshape(lon_grid_xmesh,(self.dim[0]*self.dim[1])))).reshape(self.dim[0],self.dim[1])
        ...
        #read latitude and longitude
            ...
            if self.datasource == 'LAADS DAAC':
                ...
                s=glob(self.geopath+'VNP03MOD.'+geo_matchstr +'*.nc')
                if len(s)>0:
                    self.geoname = s[0]
                    fd=h5py.File(self.geoname,'r')
                    self.latitude=np.rot90(np.array(fd.get('geolocation_data/latitude')),2)
                    self.longitude=np.rot90(np.array(fd.get('geolocation_data/longitude')),2)
                    self.dim[0]=self.latitude.shape[0]
                    self.dim[1]=self.latitude.shape[1]
                    self.imagedim=self.latitude.shape
                else:
                ...
                self.geoname = self.geopath+basename(self.l1bname)[0:-7]+'.GEO.nc'
            ...
            elif self.l8collection==2:
                utm_zone=str(self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['UTM_ZONE'])
                resol=int(self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['GRID_CELL_SIZE_REFLECTIVE'])
                ul_x=self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['CORNER_UL_PROJECTION_X_PRODUCT']
                ul_y=self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['CORNER_UL_PROJECTION_Y_PRODUCT']
                lr_x=self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['CORNER_LR_PROJECTION_X_PRODUCT']
                lr_y=self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['CORNER_LR_PROJECTION_Y_PRODUCT']
                self.dim[0]=int(self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['REFLECTIVE_LINES'])
                self.dim[1]=int(self.l8metadata['LANDSAT_METADATA_FILE']['PROJECTION_ATTRIBUTES']['REFLECTIVE_SAMPLES'])
            ...
            if lr_y > ul_y:
                ...
                resoly = -resol
            proj=pyproj.Proj('EPSG:326'+utm_zone.zfill(2))
            ...
            if self.datasource == 'LAADS DAAC':
                ...
                geo_matchstr = basename(self.l1bname)[0:-11]
                s=glob(self.geopath+geo_matchstr +'*.hdf')
                if len(s)>0:
                    self.geoname = s[0]
                    f=SD(self.geoname,SDC.READ)                    
                    self.latitude = np.rot90(f.select('Latitude')[:,:],2)
            ...
            proj = pyproj.Proj('{}'.format(utm_code))
            x, y = np.meshgrid(ul_x+resolx*np.arange(self.dim[1]),ul_y+resoly*np.arange(self.dim[0]))
            self.longitude,self.latitude = proj(x, y, inverse=True)
            self.imagedim=self.latitude.shape
        elif self.sensor == 'MERSI2':
            ...
            f=h5py.File(self.l1bname,'r')
            self.latitude = np.flip(np.array(f['/geolocation_data/latitude']),0)
            self.longitude = np.flip(np.array(f['/geolocation_data/longitude']),0)
            self.dim[0]=self.latitude.shape[0]
            self.dim[1]=self.latitude.shape[1]
            self.imagedim=self.latitude.shape
            f.close()
...
#                cwl    = data.attrs['Center_wavelength']
            ...
            solz_grid_ymesh,solz_grid_xmesh=np.meshgrid(solz_grid_yt,solz_grid_xt)
            func=interpolate.RegularGridInterpolator((solz_grid_y,solz_grid_x),solz.T,method='linear')
            # isolz=func(solz_grid_yt,solz_grid_xt)
            isolz=func((np.reshape(solz_grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(solz_grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            ...
            senz_grid_ymesh,senz_grid_xmesh=np.meshgrid(senz_grid_yt,senz_grid_xt)
            func=interpolate.RegularGridInterpolator((senz_grid_y,senz_grid_x),senz.T,method='linear')
            # isenz=func(senz_grid_yt,senz_grid_xt)
            isenz=func((np.reshape(senz_grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(senz_grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            ...
            sola_grid_ymesh,sola_grid_xmesh=np.meshgrid(sola_grid_yt,sola_grid_xt)
            func=interpolate.RegularGridInterpolator((sola_grid_y,sola_grid_x),sola.T,method='linear')
            # sola_interp=func(sola_grid_yt,sola_grid_xt)
            sola_interp=func((np.reshape(sola_grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(sola_grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            ...
            sena_grid_ymesh,sena_grid_xmesh=np.meshgrid(sena_grid_yt,sena_grid_xt)
            func=interpolate.RegularGridInterpolator((sena_grid_y,sena_grid_x),sena.T,method='linear')
            # sena_interp=func(sena_grid_yt,sena_grid_xt)
            sena_interp=func((np.reshape(sena_grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(sena_grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
...
#            idx_inval=np.isnan(senz)==1
            ...
            grid_ymesh,grid_xmesh=np.meshgrid(grid_yt,grid_xt)         
            # func = interpolate.interp2d(grid_y,grid_x,solz,kind='linear')                        
            # self.solz = func(grid_yt,grid_xt)
            func = interpolate.RegularGridInterpolator((grid_y,grid_x),solz.T,method='linear')
            self.solz = func((np.reshape(grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])            
            # func = interpolate.interp2d(grid_y,grid_x,sola,kind='linear')
            # sola_interp = func(grid_yt,grid_xt)
            func = interpolate.RegularGridInterpolator((grid_y,grid_x),sola.T,method='linear')
            sola_interp = func((np.reshape(grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            # func = interpolate.interp2d(grid_y,grid_x,senz,kind='linear')
            # self.senz = func(grid_yt,grid_xt)
            func = interpolate.RegularGridInterpolator((grid_y,grid_x),senz.T,method='linear')
            self.senz = func((np.reshape(grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            # func = interpolate.interp2d(grid_y,grid_x,sena,kind='linear')
            # sena_interp = func(grid_yt,grid_xt)
            func = interpolate.RegularGridInterpolator((grid_y,grid_x),sena.T,method='linear')
            sena_interp = func((np.reshape(grid_ymesh,(self.imagedim[0]*self.imagedim[1])), np.reshape(grid_xmesh,(self.imagedim[0]*self.imagedim[1])))).reshape(self.imagedim[0],self.imagedim[1])
            ...
            if self.datasource == 'LAADS DAAC':
                ...
                self.reflectance=np.zeros([self.dim[0],self.dim[1],nband])
                fd=h5py.File(self.l1bname,'r')
                for i in range(nband):
                    data = fd.get('observation_data/M0'+str(i+1))
                    ref=np.rot90(np.array(data),2)
                    scale=data.attrs['scale_factor'] #0.000024417415
                    offset=data.attrs['add_offset']
                    ...
                    self.reflectance[:,:,i] = ref[self.sline:self.eline,self.spixl:self.epixl] * self.l1bvgain[i] / np.pi
                fd.close()
            elif self.datasource == 'OBPG':                
                f=h5py.File(self.geoname,'r')
                data=f.get('geolocation_data/solar_zenith')
            ...
            if self.l8collection==1:
                ...
                for i in np.arange(5):
                    ds = gdal.Open(self.l1bpath + '/' + self.l8metadata['LANDSAT_METADATA_FILE']['PRODUCT_CONTENTS']['FILE_NAME_BAND_'+str(i+1)])
                    self.reflectance[:,:,i] = (ds.ReadAsArray()[self.sline:self.eline,self.spixl:self.epixl] * \
                                              self.l8metadata['LANDSAT_METADATA_FILE']['LEVEL1_RADIOMETRIC_RESCALING']['REFLECTANCE_MULT_BAND_'+str(i+1)] + \
                                              self.l8metadata['LANDSAT_METADATA_FILE']['LEVEL1_RADIOMETRIC_RESCALING']['REFLECTANCE_ADD_BAND_'+str(i+1)]) * self.l1bvgain[i] / np.pi
                    ds = None
                ds = gdal.Open(self.l1bpath + '/' + self.l8metadata['LANDSAT_METADATA_FILE']['LEVEL1_PROCESSING_RECORD']['FILE_NAME_QUALITY_L1_PIXEL'])
                flags = ds.ReadAsArray()[self.sline:self.eline,self.spixl:self.epixl]
                ds = None
                angle_file = self.l1bpath + '/' + self.l8metadata['LANDSAT_METADATA_FILE']['PRODUCT_CONTENTS']['FILE_NAME_ANGLE_COEFFICIENT']

            cloud = np.bitwise_and(flags,int('0000000000011110',2)) #maks cloud and could shadows
            self.cloud = cloud > 0
            ...
            QUANTIFICATION = int(self.l1xmlroot.find('.//QUANTIFICATION_VALUE'))
            radiometirc_offset=self.l1xmlroot.findall('.//RADIO_ADD_OFFSET')
            if self.datalevel == 'L1C':
                for i in np.arange(nband):                    
                    fid = glob(self.s2_path+'/IMG_DATA/*{}.jp2'.format(band_list[i]))[0]
                    jp = gdal.Open(fid)
                    ratiox = jp.RasterYSize/self.imagedim[0]
                    ratioy = jp.RasterXSize/self.imagedim[1]
    ...
    #                print(ratiox,ratioy)
                    if ratiox > 1 or ratioy > 1:
                        ...
                        data = downscale(jp.ReadAsArray(),self.imagedim)
                    else:
                        # data = jp[:,:]
                        data = jp.ReadAsArray()
                    jp = None    
                    if len(radiometirc_offset)==0:
                        ...
                        self.reflectance[:,:,i] = (data[self.sline:self.eline,self.spixl:self.epixl]+int(radiometirc_offset[i])) \
                                              /QUANTIFICATION * self.l1bvgain[i] / np.pi * np.cos(np.deg2rad(self.solz))
...
#                print(ratiox,ratioy)
                ...
                if len(radiometirc_offset)==0:
                    ...
                    self.reflectance[:,:,7] = (data[self.sline:self.eline,self.spixl:self.epixl]+int(radiometirc_offset[7])) \
                                          /QUANTIFICATION * self.l1bvgain[7] / np.pi * np.cos(np.deg2rad(self.solz))
...
#                print(ratiox,ratioy)
                ...
                if len(radiometirc_offset)==0:
                    ...
                    self.reflectance[:,:,8] = (data[self.sline:self.eline,self.spixl:self.epixl]+int(radiometirc_offset[8])) \
                                          /QUANTIFICATION * self.l1bvgain[8] / np.pi * np.cos(np.deg2rad(self.solz))  
            ...
            bandlist[0:55]=np.arange(9,64)#404-713nm
            bandlist[55:59]=np.arange(67,71)#736-752nm
            ...
            oci_sline = self.imagedim[0] - self.eline
            oci_eline = self.imagedim[0] - self.sline
            ...
            self.solz = (self.solz * scale).astype(Config.datatype)  # If we want float32 to save memory, need to specifically force it to be that type here
            ...
            self.senz = (self.senz * scale).astype(Config.datatype)  # If we want float32 to save memory, need to specifically force it to be that type here
            ...
            sola = (sola * scale).astype(Config.datatype)  # If we want float32 to save memory, need to specifically force it to be that type here
            ...
            sena = (sena * scale).astype(Config.datatype)

            self.relaz =sena - 180. -sola
            self.relaz[self.relaz>180.] = 360.-self.relaz[self.relaz>180.]
            self.relaz[self.relaz<-180.] = 360.+self.relaz[self.relaz<-180.]

            timestamp = self.l1bname.split('/')[-1].split('.')[1]
            dt = time.strptime(timestamp,'%Y%m%dT%H%M%S')
            # es_factor = es_dist(dt.tm_year, dt.tm_yday, dt.tm_hour*3600+dt.tm_min*60+dt.tm_sec)

            self.reflectance=np.zeros([self.dim[0], self.dim[1], nband], dtype=Config.datatype)
            ...
            wv_blue = np.array(f['/sensor_band_parameters/blue_wavelength'])
            id_blue=np.where((wv_blue>=380) & (wv_blue<600))[0]
            nblue=len(id_blue)
            blue_solar_irradiance = np.array(f['/sensor_band_parameters/blue_solar_irradiance'])
            # self.reflectance[:,:,0:nblue,] = rhot_blue[:,:,id_blue]/np.pi*(np.cos(np.deg2rad(self.solz))[:,:,None])
            self.reflectance[:,:,0:nblue] = rhot_blue[:,:,id_blue]/np.pi*self.l1bvgain[id_blue]
            del rhot_blue

            id_red=np.hstack((np.arange(17),np.arange(18,53,2),np.arange(58,77,2),np.arange(77,81),np.arange(82,86),np.arange(87,100,2), 111, 112, np.arange(114,157)))
            nred=len(id_red)
            rhot_red = np.array(f['/observation_data/rhot_red'][:,oci_sline:oci_eline,self.spixl:self.epixl])
            rhot_red = np.rollaxis(rhot_red,0,3)
            red_solar_irradiance = np.array(f['/sensor_band_parameters/red_solar_irradiance'])
            # self.reflectance[:,:,nblue:nband]=rhot_red[:,:,id_red]/np.pi*(np.cos(np.deg2rad(self.solz))[:,:,None])
            self.reflectance[:,:,nblue:nband] = rhot_red[:,:,id_red]/np.pi*self.l1bvgain[id_red]

            f.close()
            self.reflectance = np.flip(self.reflectance,0)
            self.reflectance = self.reflectance*(np.cos(np.deg2rad(self.solz))[:,:,None])
            ...
            with h5py.File(self.l1bname,'r') as f:
                # Previously, the direct georeferencing in our data was not accurate. 
                # To address this, we have corrected the georeferencing using an indirect method.
                # Indirect georeferencing may occasionally fail or be missing in some files.
                ...
                self.relaz = self.sena - 180. - self.sola
                self.relaz[self.relaz > 180.] = 360. - self.relaz[self.relaz > 180.]
                self.relaz[self.relaz < -180.] = 360. + self.relaz[self.relaz < -180.]
                ...
                for label in rhot_labels:
                    wavelength = int(re.findall(r'\d+', label)[0])
                    if wavelength in self.band:
                        rhot_labels_above_420nm.append(label)

                for i, label in enumerate(rhot_labels_above_420nm):
                    attribute = '/products/' + str(label)
                    rhot = np.array(f[attribute])
                    rhot = rhot[self.sline:self.eline,self.spixl:self.epixl] # Reset geolocation to subimage
                    self.reflectance[:, :, i] = (rhot*np.cos(np.deg2rad(self.solz)))/np.pi
