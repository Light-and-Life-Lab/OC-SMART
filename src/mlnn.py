...
#        aann_fname=self.path+self.sensor+'/'+self.sensor+'_Lt_aaNN.h5'
        ...
        for i in range(self.aann_nlayers-1):
            self.aann_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.aann_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype))
...
#        aodnn_fname=self.path+self.sensor+'/'+self.sensor+'_Lt_aodNN.h5'
        ...
        for i in range(self.aodnn_nlayers-1):
            self.aodnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.aodnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype))	
...
#        rrsnn_fname=self.path+self.sensor+'/'+self.sensor+'_Lt_rrsNN.h5'
        ...
        for i in range(self.rrsnn_nlayers-1):
            self.rrsnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.rrsnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype))
        
        if self.sensor == 'OLCI':
            # read Lrc_LrcaNN network
            LrcaNN_fname=self.path+self.sensor+'/'+self.sensor+'_Lrc_LrcaNN.h5'
            f = h5py.File(LrcaNN_fname, 'r')
            self.LrcaNN_layers=np.array(f['Layers'])
            self.LrcaNN_nlayers=len(self.LrcaNN_layers)
            self.LrcaNN_norm_in=np.array(f['Norm_in'])
            self.LrcaNN_norm_out=np.array(f['Norm_out'])
            self.LrcaNN_weights=[]                 
            self.LrcaNN_bias=[]
            for i in range(self.LrcaNN_nlayers-1):
                self.LrcaNN_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
                self.LrcaNN_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype))
...
#            self.iopnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
#            self.iopnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
#        
#        #read ocnn network
...
#            self.ocnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
#            self.ocnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
#            
        #read aphnn network
        ...
        for i in range(self.aphnn_nlayers-1):
            self.aphnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.aphnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
        ...
        for i in range(self.adgnn_nlayers-1):
            self.adgnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.adgnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
        ...
        for i in range(self.bbpnn_nlayers-1):
            self.bbpnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.bbpnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
        ...
        for i in range(self.apnn_nlayers-1):
            self.apnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.apnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype)) 
        ...
        for i in range(self.bpnn_nlayers-1):
            self.bpnn_weights.append(np.array(f['Weights/Layer'+str(i+1)], dtype=Config.datatype))
            self.bpnn_bias.append(np.array(f['Bias/Layer'+str(i+1)], dtype=Config.datatype))  
    ...
    def compute_aann(self, solz, senz, relaz, lrc, rh):
        ...
        aainput=np.zeros((ncase,int(self.aann_layers[0])), dtype=Config.datatype)
    ...
    def compute_aodnn(self, solz, senz, relaz, lrc, rh):
        print('Retrieving AOD (MLNN) ... ')
        #rebuild aodNN and compute data
        ncase=len(solz)
        nlayers=len(self.aodnn_layers)
        aodinput=np.zeros((ncase,int(self.aodnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_rrsnn(self, solz, senz, relaz, lrc,CHLfluorescence=False):
        print('Retrieving Rrs (MLNN) ... ')
        #rebuild rrsNN and compute data
        ncase=len(solz)
        nlayers=len(self.rrsnn_layers)
        rrsinput=np.zeros((ncase,int(self.rrsnn_layers[0])), dtype=Config.datatype)
        ...
        if self.sensor == 'OLCI' and CHLfluorescence:
            rrsT = np.zeros((ncase,4), dtype=Config.datatype)
            rrsN = np.zeros((ncase,2), dtype=Config.datatype)
            peak1 = (lrc[:,9]-lrc[:,7])/lrc[:,7]*100

            Lrc_a = self.compute_LrcaNN(solz, senz, relaz, lrc)

            for j in range(7,11):
               rrsT[:,j-7] = (lrc[:, j] - Lrc_a[:, j]) /np.cos(np.deg2rad(solz[:]))
            peak2 = (rrsT[:,2]-rrsT[:,0])/rrsT[:,0]*100

            R665 = rrs[:,7]/rrsT[:,0]
            R709 = rrs[:,10]/rrsT[:,3]
            R674 = (R665*(709-674) + R709*(674-665)) / (709-665)
            R681 = (R665*(709-681) + R709*(681-665)) / (709-665)

            rrsN[:,0] = rrsT[:,1]*R674	# + rrs[:,8] * (1. - 1./R674)
            rrsN[:,1] = rrsT[:,2]*R681	# + rrs[:,9] * (1. - 1./R681)

            idx=np.where((R709>0) & ((peak1>0) | (peak2>0)) & (np.sum(rrsN<0,axis=1)==0))[0]
            rrs[idx,8:10]=rrsN[idx,0:2]
    ...
    def compute_LrcaNN(self, solz, senz, relaz, lrc):       # Lrc_a
        # print('Retrieving aerosol reflectance (Lrc_a) ... ')
        #rebuild LrcaNN and compute data
        ncase=len(solz)
        nlayers=len(self.LrcaNN_layers)
        lrcainput=np.zeros((ncase,int(self.LrcaNN_layers[0])), dtype=Config.datatype)
        lrcainput[:,0]=np.cos(np.deg2rad(solz))
        lrcainput[:,1]=np.cos(np.deg2rad(senz))
        lrcainput[:,2]=np.cos(np.deg2rad(relaz))
        lrcainput[:,range(3,int(self.LrcaNN_layers[0]))]=np.log10(lrc[:,range(int(self.LrcaNN_layers[4]))])

        #normalize
        for i in range(int(self.LrcaNN_layers[0])):
            lrcainput[:,i]=2*(lrcainput[:,i]-self.LrcaNN_norm_in[i,0])/(self.LrcaNN_norm_in[i,1]-self.LrcaNN_norm_in[i,0])-1
        #compute using LrcaNN
        for i in np.arange(nlayers-1): 
            if i == 0:
                lastlayer = np.tanh(np.matmul(self.LrcaNN_weights[i],lrcainput.transpose())+self.LrcaNN_bias[i])
            elif i < nlayers-2:
                currentlayer = np.tanh(np.matmul(self.LrcaNN_weights[i],lastlayer)+self.LrcaNN_bias[i])
                lastlayer = currentlayer
            else:
                currentlayer = np.matmul(self.LrcaNN_weights[i],lastlayer)+self.LrcaNN_bias[i]
        LrcaNN_output = currentlayer.transpose()

        #denormlize
        for i in range(int(self.LrcaNN_layers[-1])):
            LrcaNN_output[:,i]=(LrcaNN_output[:,i]+1)/2*(self.LrcaNN_norm_out[i,1]-self.LrcaNN_norm_out[i,0])+self.LrcaNN_norm_out[i,0]
#        LrcaNN_output=(LrcaNN_output+1)/2*(self.LrcaNN_norm_out[1]-self.LrcaNN_norm_out[0])+self.LrcaNN_norm_out[0]
        Lrc_a=np.power(10, LrcaNN_output)
        return Lrc_a
    
    def compute_iopnn(self, rrs):
        print('Retrieving aph443, adg443 and bbp443 (MLNN) ... ')
        #rebuild iopNN and compute data
        ncase=len(rrs)
        nlayers=len(self.iopnn_layers)
        iopinput=np.zeros((ncase,int(self.iopnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_ocnn(self, rrs):
        print('Retrieving CHLa, CDOM and TSM (MLNN) ... ')
        #rebuild ocNN and compute data
        ncase=len(rrs)
        nlayers=len(self.ocnn_layers)
        ocinput=np.zeros((ncase,int(self.ocnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_aphnn(self, rrs):
        print('Retrieving aph (MLNN) ... ')
        #rebuild aphNN and compute data
        ncase=len(rrs)
        nlayers=len(self.aphnn_layers)
        aphinput=np.zeros((ncase,int(self.aphnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_adgnn(self, rrs):
        print('Retrieving adg (MLNN) ... ')
        #rebuild adgNN and compute data
        ncase=len(rrs)
        nlayers=len(self.adgnn_layers)
        adginput=np.zeros((ncase,int(self.adgnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_bbpnn(self, rrs):
        print('Retrieving bbp (MLNN) ... ')
        #rebuild bbpNN and compute data
        ncase=len(rrs)
        nlayers=len(self.bbpnn_layers)
        bbpinput=np.zeros((ncase,int(self.bbpnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_apnn(self, rrs):
        print('Retrieving at (MLNN) ... ')
        #rebuild rrsNN and compute data
        ncase=len(rrs)
        nlayers=len(self.apnn_layers)
        apinput=np.zeros((ncase,int(self.apnn_layers[0])), dtype=Config.datatype)
    ...
    def compute_bpnn(self, rrs):
        print('Retrieving bt (MLNN) ... ')
        #rebuild rrsNN and compute data
        ncase=len(rrs)
        nlayers=len(self.bpnn_layers)
        bpinput=np.zeros((ncase,int(self.bpnn_layers[0])), dtype=Config.datatype)
